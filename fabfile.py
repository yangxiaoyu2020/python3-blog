...
def build():
    ...
    with lcd(os.path.join(_current_path(), 'src')):
...
def deploy():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo('chown src-data:src-data src')
        sudo('chown -R src-data:src-data %s' % newdir)
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        r = run('ls -p -1')
        files = [s[:-1]
                 for s in RE_FILES.split(r) if s.startswith('src-') and s.endswith('/')]
        files.sort(cmp=lambda s1, s2: 1 if s1 < s2 else -1)
        r = run('ls -l src')
        ss = r.split(' -> ')
        if len(ss) != 2:
            print('ERROR: \'src\' is not a symbol link.')
            return
        current = ss[1]
